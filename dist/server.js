...
async function updateUser() {
    try {
        const res = await users.findOneAndUpdate({ username: "joysara" }, {
            $push: { likes: { id: 24 } },
        });
        // console.log(res);
    }
...
async function updateMany() {
    try {
        const res = await users.updateMany({ username: "john_doe" }, {
            $push: { likes: { id: "rep" } },
        });
        // console.log(res);
    }
...
async function deleteMany() {
    try {
        const res = await users.deleteMany({ name: "john_doe" }, { deleteAll: true });
        console.log(res);
    }
    catch (e) {
        console.log(e);
